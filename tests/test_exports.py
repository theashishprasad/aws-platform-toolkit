...
from commands.aws_drift_detector import AttributeDiff, DriftReport, DriftType, ResourceDrift
from commands.aws_healthcheck import HealthReport, HealthStatus
from commands.aws_remediation import RemediationAction, RemediationReport, RemediationStatus


def test_health_report_outputs():
    report = HealthReport(
        total=1, healthy=1, degraded=0, unhealthy=0, unknown=0, duration_seconds=0.5, checks=[]
    )
...
def test_cost_analyzer_exports():
    from commands.aws_cost_analyzer import print_cost_table, to_csv
