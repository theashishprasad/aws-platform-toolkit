...
from commands.aws_healthcheck import HealthReport
...
def test_healthcheck_cli_success(mock_run):
    ...
    mock_run.return_value = HealthReport(
        total=1, healthy=1, degraded=0, unhealthy=0, unknown=0, duration_seconds=1.0, checks=[]
    )
...
def test_healthcheck_cli_fail_on_unhealthy(mock_run):
    ...
    mock_run.return_value = HealthReport(
        total=1, healthy=0, degraded=0, unhealthy=1, unknown=0, duration_seconds=1.0, checks=[]
    )
...
def test_drift_detect_cli_no_drift(mock_compare, mock_extract, mock_load):
    ...
    mock_compare.return_value = (
        False,
        MagicMock(drift_type="MODIFIED"),
    )  # MODIFIED but not drifted bit

    # Actually mock_compare returns (is_drifted, ResourceDrift)
    # Let's be more precise
    from commands.aws_drift_detector import DriftType, ResourceDrift

    mock_compare.return_value = (
        False,
        ResourceDrift("aws_s3_bucket", "id", "s3.x", DriftType.MODIFIED),
    )
...
def test_remediate_cli(mock_restart):
    ...
    result = runner.invoke(
        app, ["remediate", "run", "--action", "restart-unhealthy-pods", "--cluster", "my-cluster"]
    )
    assert result.exit_code == 0
